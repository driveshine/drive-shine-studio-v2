...
export function FloatingButtons() {
  ...
  return (
    <>
      {/* Right side — Phone + WhatsApp pills */}
      <div className={`fixed bottom-24 right-4 z-[90] flex flex-col items-end gap-2 lg:bottom-8 lg:right-6 transition-all duration-300 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4 pointer-events-none"}`}>
        <a
          href={site.phoneHref}
      ...
      {/* Left side — Social icons always visible in dark pill */}
      <div className="hidden">
        {socials.map(({ label, href, icon: Icon, color }) => (
          <a
            ...
            rel="noreferrer"
            aria-label={label}
            className="grid size-9 place-items-center rounded-full text-white shadow transition-transform duration-200 hover:scale-110 active:scale-95"
